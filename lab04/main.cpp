...
float sinusoidalInterpolation(float a, float b, float t) {
	return a * cos(t*PI/2) + b * sin(t*PI/2);
}
...
float tweening(float a, float b, float t) {
	// return (1 - t) * a + t * b;
	// return sinusoidalInterpolation(a, b, t);
	return exponentialInterpolation(a, b, t);
	/// t from 0 to 1.
